...
public class ObjectFactory {
  ...
  /**
   * Clears the class loaders. This method should be called at the beginning of a generation run so
   * that and change to the classloading configuration will be reflected. For example, if the
   ...
   * These are classes that do not depend on any of the generator's classes or interfaces. Examples
  ...
  public static CommentGenerator createCommentGenerator(Context context) {
    ...
    if (config == null || config.getConfigurationType() == null) {
      ...
      type = CustomizationCommentGenerator.class.getName();
    } else {
      type = config.getConfigurationType();
    }
  ...
  /**
   * Creates an introspected table implementation that is only usable for validation (i.e. for a
   * context to determine if the target is ibatis2 or mybatis3).
   *
   *
