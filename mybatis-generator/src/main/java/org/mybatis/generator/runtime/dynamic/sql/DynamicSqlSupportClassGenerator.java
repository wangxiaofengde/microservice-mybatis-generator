...
public class DynamicSqlSupportClassGenerator {
  ...
  private String calculateClassName() {
    ...
    return mapperType.getPackageName() + "." + recordType.getShortNameWithoutTypeArguments() //$NON-NLS-1$
        + "DynamicSqlSupport"; //$NON-NLS-1$
  ...
  private InnerClass buildInnerTableClass(TopLevelClass topLevelClass) {
    ...
    commentGenerator.addClassAnnotation(innerClass, introspectedTable,
  ...
  private Field calculateTableDefinition(TopLevelClass topLevelClass) {
    FullyQualifiedJavaType fqjt = new FullyQualifiedJavaType(
        introspectedTable.getFullyQualifiedTable().getDomainObjectName());
    String fieldName = JavaBeansUtil
  ...
  private void handleColumn(TopLevelClass topLevelClass, InnerClass innerClass,
    ...
    commentGenerator.addFieldAnnotation(field, introspectedTable, column,
        topLevelClass.getImportedTypes());
  ...
  private String calculateInnerInitializationString(IntrospectedColumn column) {
    ...
    initializationString.append(String.format("column(\"%s\", JDBCType.%s", //$NON-NLS-1$
