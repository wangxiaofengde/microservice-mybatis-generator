...
public class ExampleGenerator extends AbstractJavaGenerator {
  ...
  private InnerClass getGeneratedCriteriaInnerClass(TopLevelClass topLevelClass) {
    ...
    for (IntrospectedColumn introspectedColumn : this.introspectedTable.getNonBLOBColumns()) {
      if (stringHasValue(introspectedColumn.getTypeHandler())) {
        criteriaLists
            .addAll(this.addtypeHandledObjectsAndMethods(introspectedColumn, method, answer));
      }
    }
  ...
  private List<String> addtypeHandledObjectsAndMethods(IntrospectedColumn introspectedColumn,
    ...
    if (introspectedColumn.getFullyQualifiedJavaType().isPrimitive()) {
      method.addParameter(new Parameter(
          introspectedColumn.getFullyQualifiedJavaType().getPrimitiveTypeWrapper(), "value")); //$NON-NLS-1$
    } else {
    ...
    if (introspectedColumn.getFullyQualifiedJavaType().isPrimitive()) {
      method.addParameter(new Parameter(
          introspectedColumn.getFullyQualifiedJavaType().getPrimitiveTypeWrapper(), "value1")); //$NON-NLS-1$
      method.addParameter(new Parameter(
          introspectedColumn.getFullyQualifiedJavaType().getPrimitiveTypeWrapper(), "value2")); //$NON-NLS-1$
    } else {
  ...
  private Method getSingleValueMethod(IntrospectedColumn introspectedColumn, String nameFragment,
    ...
    if (introspectedColumn.getFullyQualifiedJavaType().isPrimitive()
        && !this.introspectedTable.isJava5Targeted()) {
