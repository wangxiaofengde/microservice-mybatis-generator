...
public class DatabaseIntrospector {
  ...
  private void calculatePrimaryKey(FullyQualifiedTable table, IntrospectedTable introspectedTable) {
    ...
    try {
      rs = this.databaseMetaData.getPrimaryKeys(table.getIntrospectedCatalog(),
          table.getIntrospectedSchema(), table.getIntrospectedTableName());
    } catch (SQLException e) {
      this.closeResultSet(rs);
      this.warnings.add(getString("Warning.15")); //$NON-NLS-1$
      return;
    }
  ...
  private void reportIntrospectionWarnings(IntrospectedTable introspectedTable,
    ...
    for (ColumnOverride columnOverride : tableConfiguration.getColumnOverrides()) {
      if (introspectedTable.getColumn(columnOverride.getColumnName()) == null) {
        this.warnings.add(getString("Warning.3", //$NON-NLS-1$
    ...
    for (String string : tableConfiguration.getIgnoredColumnsInError()) {
      this.warnings.add(getString("Warning.4", //$NON-NLS-1$
    ...
    if (generatedKey != null && introspectedTable.getColumn(generatedKey.getColumn()) == null) {
      if (generatedKey.isIdentity()) {
        this.warnings.add(getString("Warning.5", //$NON-NLS-1$
            generatedKey.getColumn(), table.toString()));
      } else {
        this.warnings.add(getString("Warning.6", //$NON-NLS-1$
            generatedKey.getColumn(), table.toString()));
      }
    }
    ...
    for (IntrospectedColumn ic : introspectedTable.getAllColumns()) {
      if (JavaReservedWords.containsWord(ic.getJavaProperty())) {
        this.warnings.add(getString("Warning.26", //$NON-NLS-1$
  ...
  public List<IntrospectedTable> introspectTables(TableConfiguration tc) throws SQLException {
    ...
    if (columns.isEmpty()) {
      this.warnings.add(getString("Warning.19", tc.getCatalog(), //$NON-NLS-1$
      ...
      while (tableColumns.hasNext()) {
        ...
        if (tc.isColumnIgnored(introspectedColumn.getActualColumnName())) {
          ...
          if (this.logger.isDebugEnabled()) {
            this.logger.debug(getString("Tracing.3", //$NON-NLS-1$
    ...
    for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
      for (IntrospectedColumn introspectedColumn : entry.getValue()) {
        ...
        if (fullyQualifiedJavaType != null) {
          ...
          introspectedColumn
              .setJdbcTypeName(this.javaTypeResolver.calculateJdbcTypeName(introspectedColumn));
        } else {
          ...
          if (warn) {
            introspectedColumn
                .setFullyQualifiedJavaType(FullyQualifiedJavaType.getObjectInstance());
    ...
    for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
      for (IntrospectedColumn introspectedColumn : entry.getValue()) {
        ...
        if (columnOverride != null) {
          if (this.logger.isDebugEnabled()) {
            this.logger.debug(getString("Tracing.4", //$NON-NLS-1$
                introspectedColumn.getActualColumnName(), entry.getKey().toString()));
          }
  ...
  private Map<ActualTableName, List<IntrospectedColumn>> getColumns(TableConfiguration tc)
    ...
    if (this.logger.isDebugEnabled()) {
      ...
      this.logger.debug(getString("Tracing.1", fullTableName)); //$NON-NLS-1$
    }

    ResultSet rs = this.databaseMetaData.getColumns(localCatalog, localSchema, localTableName, "%"); //$NON-NLS-1$
    ...
    while (rs.next()) {
      ...
      if (this.logger.isDebugEnabled()) {
        this.logger.debug(getString("Tracing.2", //$NON-NLS-1$
            introspectedColumn.getActualColumnName(),
    ...
    if (answer.size() > 1 && !stringContainsSQLWildcard(localSchema)
      ...
      this.warnings.add(getString("Warning.25", //$NON-NLS-1$
  ...
  private void enhanceIntrospectedTable(IntrospectedTable introspectedTable) {
    try {
      ...
      ResultSet rs = this.databaseMetaData.getTables(fqt.getIntrospectedCatalog(),
      ...
      this.warnings.add(getString("Warning.27", e.getMessage())); //$NON-NLS-1$
    }
  }
}
