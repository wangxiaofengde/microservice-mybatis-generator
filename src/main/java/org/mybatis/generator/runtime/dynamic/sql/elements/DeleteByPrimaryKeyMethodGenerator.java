...
public class DeleteByPrimaryKeyMethodGenerator extends AbstractMethodGenerator {
  ...
  public MethodAndImports generateMethodAndImports() {
    ...
    method
        .addBodyLine("return DeleteDSL.deleteFromWithMapper(this::delete, " + tableFieldName + ")"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    return MethodAndImports.withMethod(method).withImports(imports).withStaticImports(staticImports)
        .build();
  }

  @Override
  public boolean callPlugins(Method method, Interface interfaze) {
    return context.getPlugins().clientDeleteByPrimaryKeyMethodGenerated(method, interfaze,
        introspectedTable);
  }
